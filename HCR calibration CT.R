...
param_draws_CT = as.data.frame(1:n_drawz)
names(param_draws_CT)[1] <- 'tripid'
param_draws_CT$beta_sqrt_sf_keep = rnorm(n_drawz, mean = 0.7553103, sd = 1.304717)
param_draws_CT$beta_sqrt_sf_release = rnorm(n_drawz, mean = 0.063097 , sd = 0.2389134 )
param_draws_CT$beta_sqrt_bsb_keep = rnorm(n_drawz, mean = 0.2796952 , sd = 0.3044652 )
param_draws_CT$beta_sqrt_bsb_release = rnorm(n_drawz, mean = 0.069036 , sd = 0 )
param_draws_CT$beta_sqrt_scup_catch = rnorm(n_drawz, mean = 0.0182875 , sd = 0)
param_draws_CT$beta_opt_out = rnorm(n_drawz, mean = -1.822518, sd = 2.040809 )
param_draws_CT$beta_cost = rnorm(n_drawz, mean = -0.0114263, sd = 0)
...
for(p in levels(periodz)){
  ...
  costs_new_CT[[p]] = subset(trip_data, select=c(tripid, cost, catch_draw, tot_keep_sf, tot_rel_sf,
                                                 tot_keep_bsb,tot_rel_bsb,tot_scup_catch  ))
  
  names(costs_new_CT[[p]])[names(costs_new_CT[[p]]) == "tot_keep_sf"] = "tot_keep_sf_base"
  names(costs_new_CT[[p]])[names(costs_new_CT[[p]]) == "tot_rel_sf"] = "tot_rel_sf_base"
  ...
  names(costs_new_CT[[p]])[names(costs_new_CT[[p]]) == "tot_scup_catch"] = "tot_cat_scup_base"
