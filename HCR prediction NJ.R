...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sf_catch_data <- data.frame(readRDS("pred_catch_data_NJ.rds"))                                                                            
    ...
    ###
    scup_nbs<- subset(read_csv("nb_params.csv",  show_col_types = FALSE), state==34 & draw==x)
...
for(p in levels(periodz)){
  ...
  for(d in 1:1) {
    ...
    #param_draws_NJ_prediction <- subset(param_draws_NJ, parameter_draw==1)
