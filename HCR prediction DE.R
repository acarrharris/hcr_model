...
#     By region (MA-NY, NJ, DE-NC, VA-NC): utility parameters and target species 
...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sf_catch_data <- data.frame(readRDS("pred_catch_data_DE.rds"))                                                                            
...
for(p in levels(periodz)){
  ...
  cost_data <- subset(costs_new_all_DE, period == p, select=-c(period))
  ...
  for(d in 1:1) {
    ...
    #param_draws_DE_prediction <- subset(param_draws_DE, parameter_draw==1)
