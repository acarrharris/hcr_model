...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sf_catch_data <- data.frame(readRDS("pred_catch_data_NY.rds"))                                                                            
    tot_sf_catch <- sf_catch_data$sf_pred_cat
    tot_bsb_catch <- sf_catch_data$bsb_pred_cat
    
    # scup_nbs <-readRDS("nb_catch_parameters_scup_NY_21.rds")
    # scup_mu_param<-scup_nbs$estimate['mu']
    # scup_size_param<- scup_nbs$estimate['size'] 
    
    ###
    scup_nbs<- subset(read_csv("nb_params.csv",  show_col_types = FALSE), state==36 & draw==x)
...
for(p in levels(periodz)){
  ...
  cost_data <- subset(costs_new_all_NY, period == p, select=-c(period))
  ...
  for(d in 1:1) {
    ...
    #param_draws_NY_prediction <- subset(param_draws_NY, parameter_draw==1)
