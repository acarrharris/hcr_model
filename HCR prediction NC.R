...
for(p in levels(periodz)){
  ...
  for(i in 1:10) {
    ...
    sf_catch_data <- data.frame(readRDS("pred_catch_data_NC.rds"))                                                                            
...
for(p in levels(periodz)){
  ...
  cost_data <- subset(costs_new_all_NC, period == p, select=-c(period))
  ...
  for(d in 1:1) {
    ...
    #param_draws_NC_prediction <- subset(param_draws_NC, parameter_draw==1)
